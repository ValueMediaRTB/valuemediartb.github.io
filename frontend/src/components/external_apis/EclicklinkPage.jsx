...
const EclicklinkPage = ({ onBack }) => {
  ...
  const exportOffers = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setResultTitle("eclicklink/exportOffers failed! Received response " + response.status);
      } else {
        const data = await response.json();
        setResultTitle("Export offers successful!");
        setResultContainer("Downloading eclicklinkOffers.csv...");
  ...
  return (
    <Container fluid className="p-4">
      <div className="mb-4">
        <h3 style={{ margin: '8px' }}>eclicklink API</h3>
        <Button 
