...
const aggregationSchema = new mongoose.Schema({
  ...
  exads_camp_id: {type: Number, index: true},
...
// exads_camp_id and zone must exist
...
aggregationSchema.index({
  ...
  unique: true,
  partialFilterExpression: { exads_camp_id: { $exists: true },
    zone: { $exists: false } }
});
...
aggregationSchema.index({
  ...
  unique: true,
  partialFilterExpression: { zone: { $exists: true },
    exads_camp_id: { $exists: false } }
});
