...
//let binomTrafficSourcesParam = binomTrafficSources.map(id => `ids%5B%5D=${id}`).join('&');
let binomTrafficSourcesParam = [437,436,435,434,432,430,421,402,388,381,363,341,313,303,284,282].join("&ids%5B%5D=")
...
const fetchBinomDataForDateRange = async (reportTypes, date, filters) => {
  ...
  if(filters && filters.traffic_sources)
    binomTrafficSourcesParam = filters.traffic_sources.map(id => `ids%5B%5D=${id}`).join('&');
  else
    binomTrafficSourcesParam = 'ids%5B%5D='+binomTrafficSourcesParam;
          ...
          request: queue.add(async () => {
            try {
              ...
              const errorMsg = error.response?.status 
                ...
                : error.message;
              console.error(`Error fetching offset ${currentOffset}:`, errorMsg);
              return {
                data: [],
                offset: currentOffset
              };
